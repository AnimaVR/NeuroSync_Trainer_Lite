...
def load_arkit_generator(arkit_generator_model_path, device):
    
    import torch 
    from utils.model import Seq2Seq, Encoder, Decoder
    ...
    model = Seq2Seq(Encoder(**inputconfigs), Decoder(**outputconfigs), device)

    checkpoint_dict = torch.load(arkit_generator_model_path)
...
class Seq2Seq_MH(nn.Module):
    def __init__(self, arkit_generator_model_path, with_emotions, freeze_arkit_generator, device):
        ...
        # Freeze all parameters of the arkit_generator
        if freeze_arkit_generator:
